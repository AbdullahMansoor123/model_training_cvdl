...
def get_data(batch_size, data_root, num_workers=4, data_augmentation=False):
    mean, std = get_mean_std(data_root, num_workers)

    train_transformer = image_preprocess_transforms()

    image_common_transformer = image_common_transforms(mean, std)

    train_data_path = os.path.join(data_root, 'training')
    train_loader = data_loader(train_data_path, transform=train_transformer, batch_size=batch_size, shuffle=True, num_workers=num_workers)

    test_data_path = os.path.join(data_root, 'validation')
    test_loader = data_loader(test_data_path, transform=image_common_transformer, batch_size=batch_size, shuffle=False, num_workers=num_workers)
...
def get_targets_probs(model, data_root, mean, std):
    batch_size = 15
